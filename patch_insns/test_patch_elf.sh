...
function is_patcher_successful {
    if [[ $? -gt 0 ]]
    then
        echo "ERROR: parse_elf failed to run with $1"
        exit
    fi
}
...
function is_output_elf_correct {
    if [[ $? -gt 0 ]]
    then
        echo "ERROR: parse_elf $1 resulted in invalid ELF output"
        exit
    fi
}
...
perform_test examples/lwt_len_hist_kern.o sample-insns/lwt_len_hist.insns len_hist results/lwt_len_hist_kern_patched.o 
